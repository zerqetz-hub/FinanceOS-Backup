...
FinanceOS is a multi-user personal finance web app (Node.js + Express + PostgreSQL). The server is an SPA backend — it serves static files from `public/` and exposes a REST API under `/api/`.
...
- **[auth.js](auth.js)** — Password hashing (PBKDF2/SHA-512, 100k iterations), session token management via `fos_session` HttpOnly cookie, IP-based rate limiting (5 attempts, 15-min lockout), and the `requireAuth` Express middleware.
